...
install_project() {
    if [[ "$INSTALL_AFTER_BUILD" == true ]]; then
        ...
        # 生成并复制更新的 run.sh 脚本，适配安装目录
        cat > "$INSTALL_TARGET/run.sh" << 'RUN_SCRIPT_EOF'
#!/bin/bash

# Run script for SCADASystem - Installation Version

# Get the directory where the script is located
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

# For installation, executable is always in the same directory as the script
EXECUTABLE_DIR="$SCRIPT_DIR"

# Set LD_LIBRARY_PATH to include the lib directory and Qt libraries
export LD_LIBRARY_PATH="$EXECUTABLE_DIR/lib:$LD_LIBRARY_PATH"
...
"$EXECUTABLE_DIR/SCADASystem"
RUN_SCRIPT_EOF
        chmod +x "$INSTALL_TARGET/run.sh"
        print_info "已生成安装目录的 run.sh 启动脚本"
